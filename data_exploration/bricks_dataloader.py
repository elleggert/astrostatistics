...
df_south = pd.DataFrame(columns=['BrickID', 'ObjectID','RA', 'DEC', 'South', 'Target_type'])
...
for no, brickname in enumerate(bricknames_south_sample):
    ...
    for i in range(len(flux_g)):
        ...
        if isLRG(gflux=flux_g[i], rflux=flux_r[i], zflux=flux_z[i], w1flux=flux_w1[i], w2flux=flux_w2[i],
            ...
            df_south = df_south.append({'BrickID': brickid, 'ObjectID': objid[i],'RA': ra[i], 'DEC': dec[i],
...
df_south.to_csv('../bricks_data/galaxy_catalogue_sample_south.csv', index=False)
...
df_north = pd.DataFrame(columns=['BrickID', 'ObjectID','RA', 'DEC', 'South', 'Target_type'])
...
for no, brickname in enumerate(bricknames_north_sample):
    ...
    for i in range(len(flux_g)):
        ...
        if isLRG(gflux=flux_g[i], rflux=flux_r[i], zflux=flux_z[i], w1flux=flux_w1[i], w2flux=flux_w2[i],
                 ...
                 gaiagmag=gaia_g_mag[i], gnobs=nobs_g[i], rnobs=nobs_r[i], znobs=nobs_z[i], maskbits=maskbits[i],
                 zfibertotflux=fibertotflux_z[i], primary=None, south=south):
            df_north =  df_north.append({'BrickID': brickid, 'ObjectID': objid[i],'RA': ra[i], 'DEC': dec[i],
                             'South': south, 'Target_type': 1}, ignore_index=True)
            continue
...
df_north.to_csv('../bricks_data/galaxy_catalogue_sample_north.csv', index=False)
